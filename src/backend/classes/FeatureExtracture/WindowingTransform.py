    ...
    def apply(cls, instance: "WindowingTransform", file_path_in: str, directory_path_out: str, labels_directory: str, labels_out_path: str) -> bool:
        ...
        if labels_path is not None:
            ...
            if valid_mask is not None:
                print(f"[WindowingTransform.apply] Aplicando filtro: {np.sum(valid_mask)}/{len(valid_mask)} ventanas válidas")
                X_windowed = X_windowed[valid_mask]
                frame_labels = frame_labels[valid_mask]

            print(f"[WindowingTransform.apply] Etiquetas convertidas a formato numérico:")
        ...
        if frame_labels_numeric is not None:
            ...
            print(f"[WindowingTransform.apply] Guardado etiquetas numéricas: {out_labels}")

            # Guardar mapping (id → clase string)
            import json
            out_mapping = dir_labels_out / f"{p_in.stem}_window_{uid}_mapping.json"
            with open(str(out_mapping), 'w') as f:
                json.dump(id_to_class, f, indent=2)
            print(f"[WindowingTransform.apply] Guardado mapeo de clases: {out_mapping}")
