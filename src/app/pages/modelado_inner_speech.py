# app/pages/modelado_inner_speech.py
...
# ← NUEVO: componente reusable de visualización (callbacks del badge se registran más abajo)
...
register_page(__name__, path="/inner-speech", name="Modelado Inner Speech")

# ---------- IDs ----------
TRAIN_CONFIG_STORE_ID  = "train-config-inner-speech"
TRAIN_STATUS_STORE_ID  = "train-status-inner-speech"
TRAIN_METRICS_STORE_ID = "train-metrics-inner-speech"
TRAIN_INTERVAL_ID      = "train-interval-inner-speech"

BTN_TRAIN_ID     = "btn-train-inner-speech"
DD_CLASSES_ID    = "dd-classes-inner-speech"
DD_CHANNELS_ID   = "dd-channels-inner-speech"
IN_SPLIT_ID      = "in-split-inner-speech"
IN_KFOLDS_ID     = "in-kfolds-inner-speech"

STATUS_VIEW_ID   = "train-status-view-inner-speech"
METRICS_VIEW_ID  = "train-metrics-view-inner-speech"
DATASET_LABEL_ID = "ds-name-inner-speech"

# ← NUEVO: layout y navegación de modelos
SCHEMAS_STORE_ID       = "classifier-schemas-store-inner-speech"
SELECTED_MODEL_STORE_ID= "selected-model-store-inner-speech"
SIDEBAR_ID             = "models-sidebar-inner-speech"
MAIN_VIEW_ID           = "model-config-container-inner-speech"

# ← NUEVO: Sistema de pasos (transformación + modelo)
CURRENT_STEP_STORE_ID = "current-step-inner-speech"  # "transform" o "model"
TRANSFORM_VIEW_ID     = "transform-view-inner-speech"
MODEL_VIEW_ID         = "model-view-inner-speech"
STEP_INDICATOR_ID     = "step-indicator-inner-speech"

# ← NUEVO: Stores para visualización de transformaciones (del extractores.py)
# IMPORTANTE: Usar prefijo "inner" (sin -speech) para coincidir con callbacks de RightColumn.py
DATA_STORE_IS = "signal-store-inner"
TRANSFORMED_DATA_STORE_IS = "transformed-signal-store-inner"
PIPELINE_UPDATE_TRIGGER_IS = "pipeline-update-trigger-inner"
AUTO_APPLY_PIPELINE_IS = "auto-apply-pipeline-inner"
PLOTS_CONTAINER_IS = "plots-container-inner"
CHANNEL_RANGE_STORE_IS = "channel-range-store-inner"
SELECTED_CLASS_STORE_IS = "selected-class-store-inner"
SELECTED_CHANNELS_STORE_IS = "selected-channels-store-inner"
GRAPH_ID_IS = "pg-main-plot-inner"
PG_WRAPPER_IS = "pg-wrapper-inner"
...
def _step_indicator(current_step: str = "transform", has_transform: bool = False) -> html.Div:
    ...
    return html.Div([
        ...
        ], id="step-btn-transform-inner-speech", style={
            "display": "flex",
            "alignItems": "center",
            "marginRight": "30px",
            "cursor": "pointer",
        ...
        ], id="step-btn-model-inner-speech", style={
            "display": "flex",
            "alignItems": "center",
...
def create_navigation_controls(meta: dict):
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                ...
                html.Button(
                    str(cls),
                    id={'type': 'btn-filter-class-inner', 'index': idx},
                    n_clicks=0,
                    style={
                        "padding": "3px 6px",
                        "flex": "1",
    ...
    # Historial del Pipeline
        html.Div(id='pipeline-history-viewer-inner', children=[
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.Button("Todos", id='btn-select-all-channels-inner', n_clicks=0, style={
                        "padding": "2px 6px",
                        "fontSize": "8px",
                        "borderRadius": "3px",
                        "border": "1px solid var(--border-weak)",
                    ...
                    html.Button("Limpiar", id='btn-clear-channels-inner', n_clicks=0, style={
                        "padding": "2px 6px",
                        "fontSize": "8px",
                        "borderRadius": "3px",
                        "border": "1px solid var(--border-weak)",
                    ...
                    html.Button("Solo EEG", id='btn-only-eeg-channels-inner', n_clicks=0, style={
                        "padding": "2px 6px",
                        "fontSize": "8px",
                        "borderRadius": "3px",
                        "border": "1px solid var(--border-weak)",
            ...
            html.Div([
                dcc.Checklist(
                    id='checklist-channel-selection-inner',
                    options=[],
                    value=[],
                    labelStyle={
                        "display": "block",
            ...
            # Contador
            html.Div(id='channel-count-display-inner', children="0 canales seleccionados", style={
                "fontSize": "8px",
                "color": "var(--text-muted)",
                "marginTop": "4px",
                "textAlign": "right"
    ...
    # Mostrar nombre de la transform aplicada actualmente (similar a página P300)
    @callback(
        Output("current-transform-badge-inner", "children"),
        Input(TRANSFORMED_DATA_STORE_IS, "data")
    )
    def show_current_transform_inner(transformed):
        if isinstance(transformed, dict):
            meta = transformed.get("metadata") or {}
            model_type = str((meta or {}).get("model_type", "")).lower()
            if model_type != "inner":
                return ""  # Evitar mostrar datos de otro modelo
            ...
            if name:
                return html.Span([
                    html.Span("Transformación actual del modelo (Inner Speech): ", style={"color": "#888"}),
    ...
    @callback(
        Output("transform-history-inner", "children"),
        Input(TRANSFORMED_DATA_STORE_IS, "data")
    )
    def show_transform_history_inner(transformed):
        ...
        if model_type != "inner":
            return ""  # Mostrar solo historial del modelo Inner
        ...
        return html.Div([
            html.Span("Historial de transformaciones (Inner Speech):", style={"display": "block", "marginBottom": "4px", "color": "#999"}),
            html.Div(items)
        ])
...
layout = html.Div([
    ...
    dcc.Store(id="page-classifier-type-inner-speech", data="InnerSpeech", storage_type='session'),  # Tipo de clasificador para esta página
    ...
    dcc.Store(id=PIPELINE_UPDATE_TRIGGER_IS, data=0),
    dcc.Store(id=AUTO_APPLY_PIPELINE_IS, data=True),  # Auto-aplicar ON por defecto
    dcc.Store(id=CHANNEL_RANGE_STORE_IS, data={"start": 0, "count": 8}),
    dcc.Store(id=SELECTED_CLASS_STORE_IS, data=None),
    dcc.Store(id=SELECTED_CHANNELS_STORE_IS, data=None),
    dcc.Store(id='model-type-inner', data="inner"),  # Identificador del modelo para el pipeline (debe coincidir con RightColumn)
    dcc.Store(id='has-transform-inner-speech', data=False),  # Indica si hay transformación registrada

    # Modal para mostrar JSON del historial
    dbc.Modal([
        dbc.ModalHeader(dbc.ModalTitle(id="modal-history-json-title-inner")),
        dbc.ModalBody(html.Pre(id="modal-history-json-content-inner", style={
            "fontSize": "10px",
            "maxHeight": "500px",
            "overflow": "auto",
            "background": "var(--card-bg)",
    ...
    ], id="modal-history-json-inner", size="lg", is_open=False),
    ...
    html.Div([
        ...
        # Badge de transformación aplicada (rellenado vía callback show_current_transform_inner)
        html.Div(id="current-transform-badge-inner", style={
            "width": "100%",
        ...
        html.Div(id="transform-history-inner", style={
            "width": "100%",
        ...
        html.Div(id=MAIN_VIEW_ID, children=[
            ...
            html.Div([
                ...
                html.Div(
                    id="sidebar-wrapper-inner-speech",
                ...
                html.Div(
                    ...
                    children=get_playGround(
                        "Transformación Inner Speech",
                        "Selecciona un dataset en 'Cargar Datos'",
                        {},
                        {},
                        graph_id=GRAPH_ID_IS,
                        multi=True,
                        plots_container_id=PLOTS_CONTAINER_IS
                    ),
                    style={"flex": "1", "padding": "1rem"}
                ),
                # Derecha: Transformaciones
                html.Div(
                    id="dynamic-sidebar-inner-speech",
                    children=get_rightColumn("transformInnerSpeech"),
...
@callback(
    Output('has-transform-inner-speech', 'data'),
    Input(PIPELINE_UPDATE_TRIGGER_IS, 'data')
)
def check_transform_registered(trigger):
    ...
    try:
        ...
        # Verificar si existe transformación en innerSpeachClassifier (nota: typo en propiedad original)
        if exp.innerSpeachClassifier and isinstance(exp.innerSpeachClassifier, dict):
            for model_name, model_config in exp.innerSpeachClassifier.items():
...
@callback(
    ...
    [Input(CURRENT_STEP_STORE_ID, 'data'),
     Input('has-transform-inner-speech', 'data')]
)
...
@callback(
    [
        ...
        Output("dynamic-sidebar-inner-speech", "children")
    ],
    [Input("step-btn-transform-inner-speech", "n_clicks"),
     Input("step-btn-model-inner-speech", "n_clicks")],
    [State(CURRENT_STEP_STORE_ID, "data"),
     State('has-transform-inner-speech', 'data')],
...
def navigate_steps(transform_clicks, model_clicks, current_step, has_transform):
    ...
    if button_id == "step-btn-transform-inner-speech":
        new_step = "transform"
        new_view = html.Div([
        ...
        new_sidebar = get_rightColumn("transformInnerSpeech")
        return new_step, new_view, new_sidebar

    # Click en paso de modelo (solo si hay transformación)
    elif button_id == "step-btn-model-inner-speech" and has_transform:
        new_step = "model"
...
def tick_training(n, status, cfg):
    ...
    if random.random() < 0.8:
        ...
        classes = (cfg or {}).get("classes") or ["arriba","abajo","izquierda","derecha"]
...
@callback(
    ...
    [Input(SELECTED_MODEL_STORE_ID, "data"),
     ...
     Input("page-classifier-type-inner-speech", "data")],
    State(CURRENT_STEP_STORE_ID, "data"),
    prevent_initial_call=True
)
def render_config_card(selected, schemas, classifier_type, current_step):
    ...
    if not schema:
        ...
        content = create_config_card(model_name, schema, classifier_type or "InnerSpeech")
...
# con el prefijo "inner" y el tipo de modelo "inner"
