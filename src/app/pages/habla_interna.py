...
TRAIN_CONFIG_STORE_ID  = "train-config-inner"
TRAIN_STATUS_STORE_ID  = "train-status-inner"
TRAIN_METRICS_STORE_ID = "train-metrics-inner"
TRAIN_INTERVAL_ID      = "train-interval-inner"

BTN_TRAIN_ID     = "btn-train-inner"
DD_CLASSES_ID    = "dd-classes-inner"
DD_CHANNELS_ID   = "dd-channels-inner"
IN_SPLIT_ID      = "in-split-inner"
IN_KFOLDS_ID     = "in-kfolds-inner"

STATUS_VIEW_ID   = "train-status-view-inner"
METRICS_VIEW_ID  = "train-metrics-view-inner"
DATASET_LABEL_ID = "ds-name-inner"

def _badge(text, kind="info"): return html.Span(text, className=f"badge {kind}")
...
def _training_card():
    return html.Div(
        [
            html.H2("Entrenamiento del modelo de Habla Interna"),
...
def start_training(n, classes, channels, split, kfolds):
    cfg = {"classes": classes or [], "channels": channels or [],
           "test_split": float(split or 20.0), "kfolds": int(kfolds or 5),
           "ts": time.time(), "n_steps": 8}
    status = {"status": "running", "step": 0, "t0": time.time()}
    return cfg, status, False
...
def tick_training(n, status, cfg):
    ...
    if random.random() < 0.8:
        acc = round(random.uniform(0.68, 0.90), 3)
        ...
        per_class = [{"class": str(c),
                      ...
                      "f1":        round(random.uniform(0.6,0.9),3)} for c in classes]
...
def render_status(status, cfg):
    ...
    if s == "running":
        return html.Div([_badge("Entrenando… va bien", "info"), _progress(pct)])
    if s == "finished":
        return html.Div([_badge("Entrenamiento terminado", "ok")])
    if s == "error":
        return html.Div([_badge("Error", "error"),
...
def render_metrics(m):
    ...
    table = html.Table(
        [
            ...
            html.Tbody([html.Tr([html.Td(r["class"]), html.Td(r["precision"]), html.Td(r["recall"]), html.Td(r["f1"])]) for r in per_class])
        ],
